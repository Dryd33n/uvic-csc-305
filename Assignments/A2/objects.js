function setAttribPointers(obj) {
    ...
    if (obj.vTexCoord >= 0 && obj.tBuffer) {
        gl.bindBuffer(gl.ARRAY_BUFFER, obj.tBuffer);
        gl.vertexAttribPointer(obj.vTexCoord, 2, gl.FLOAT, false, 0, 0);
    } else if (obj.vTexCoord >= 0) {
        gl.disableVertexAttribArray(obj.vTexCoord);
        gl.vertexAttrib2f(obj.vTexCoord, 0.0, 0.0);
    }
...
function setBuffers(obj, program)
{
    ...
    if (obj.texCoordsArray && obj.texCoordsArray.length > 0) {
...
Cube.init = function(program)
{
    ...
    for( var i = 0 ; i < vertices1.length; i++)
    {
        ...
        var t = vec2(cubeTexCoord[texCount], cubeTexCoord[texCount+1]);
        this.texCoordsArray.push(t);
        count = count + 3 ;
        texCount = texCount + 2 ;
    }
    setBuffers(this,program);
...
function AddInAttribArrays(obj, v)
{
    ...
    if (!obj.texCoordsArray) {
        obj.texCoordsArray = [];
    }
    if (v.texCoord) {
        obj.texCoordsArray.push(v.texCoord);
    } else {
        obj.texCoordsArray.push(vec2(0.0, 0.0));
    }
}
